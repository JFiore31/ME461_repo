...
//#############################################################################
//connections:
//quadrotor pwms: GPIO0,1,2,3
//spi the same
//LED:34
//V+:5v for regulator
//motorpwr: 4v for motor
...
uint16_t StartKalman = 0;
float accelx_kalman = 0;
float accely_kalman = 0;
float accelz_kalman = 0;
float gyrox_kalman = 0;
float gyroy_kalman = 0;
float gyroz_kalman = 0;
int16_t firstinSPI = 1;
//
// Function Prototypes
//
...
void main(void)
{
    ...
    ConfigCpuTimer(&CpuTimer1, 60, 2000);
...
void SPI_RXint(void) {
    ...
    case 5:
        ...
        if(calibration_state == 0){
            ...
            accelx_cal = accelx - (accelx_offset);
            accely_cal = accely - (accely_offset);
            accelz_cal = accelz - (accelz_offset);

            gyrox_cal  = gyrox - gyrox_offset;
            gyroy_cal  = gyroy - gyroy_offset;
            gyroz_cal  = gyroz - gyroz_offset;
            StartKalman = 1;
            gyrox_kalman = gyrox_cal;
            gyroy_kalman = gyroy_cal;
            gyroz_kalman = gyroz_cal;
            accelx_kalman = accelx_cal;
            accely_kalman = accely_cal;
            accelz_kalman = accelz_cal;
