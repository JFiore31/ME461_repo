...
//bcy hw1 predefinition for saturate function
float saturate(float input, float saturation_limit);

//bcy hw1 Global variables
float sinvalue = 0;
float time = 0;
float ampl = 3.0;
...
//			serial_printf(&SerialA,"timeint %d var16 %d var32 %d var16 %d\r\n",timeint, test16_1, test32, test16_2);
            serial_printf(&SerialA,"timeint %ld var16 %d var32 %ld var16 %d\r\n",timeint, test16_1, test32, test16_2);
            UARTPrint = 0;
        }
    }
}

//bcy hw1 saturate function
float saturate(float input, float saturation_limit){
    if(input < 0){
        // input is negative
        if (-saturation_limit > input) {
...
__interrupt void cpu_timer2_isr(void)
{
	...
	// bcy HW1 increments for the 3 test variables
	if ((CpuTimer2.InterruptCount % 250) == 0) {
	        test32 ++;
	        test16_1 ++;
	        test16_2 ++;
	}
