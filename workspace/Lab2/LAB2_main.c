...
//JMF Extern Variable for displaying LEDs when buttons are pressed
extern int16_t PushButtonTest = 0;
...
void main(void)
{
    ...
    //JMF timers 0 and 1 commented out since they are not being used.
    //Enable bit is never set to enable so they are not used
    ...
    {
        if (UARTPrint == 1 ) {
            serial_printf(&SerialA,"Num Timer2:%ld Num SerialRX: %ld Switch Status: %d\r\n",numTimer2calls,numRXA,PushButtonTest);
            UARTPrint = 0;
        }
    }
}
